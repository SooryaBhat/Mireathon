...
export default function RegistrationFlow() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (!fullName.trim() || !phoneNumber.trim() || !email.trim() || !password) {
      setErrorMsg("All fields are required (Full Name, Phone Number, Email, Password).");
      return;
    }
    ...
    setLoading(true);
    const res = await signUpStudent(fullName.trim(), phoneNumber.trim(), email.trim(), password);
    setLoading(false);
          ...
          <AnimatePresence mode="wait">
                ...
                <form onSubmit={handleSignUp} className="space-y-4 max-w-md mx-auto">
                  ...
                  <div>
                    ...
                    <div className="relative flex items-center">
                      <span className="absolute left-4 font-mono text-cyan-400 text-sm font-bold select-none">+91</span>
                      <input
                        type="tel"
                        required
                        ...
                        className="w-full pl-14 pr-4 py-3 rounded-xl bg-black/60 border border-purple-500/40 text-white placeholder-gray-500 focus:outline-none focus:border-cyan-400 text-sm font-mono tracking-wider"
                      />
                    </div>
                    <span className="text-[10px] font-mono text-gray-400 mt-1 block">
                      // Required for hackathon contact &amp; event updates (+91 India default)
                    </span>
                  </div>
            ...
            {step === "DASHBOARD" && activeTeam && (
              ...
              >
                ...
                {/* ── DEDICATED SECTION: OFFICIAL WHATSAPP GROUP ── */}
                <div className="p-5 sm:p-6 rounded-3xl bg-gradient-to-r from-emerald-950/70 via-black/90 to-purple-950/70 border border-emerald-500/40 shadow-[0_0_25px_rgba(37,211,102,0.15)] backdrop-blur-md">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="space-y-1 max-w-xl">
                      <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-950 border border-emerald-500/40 text-emerald-300 font-mono text-[10px] uppercase font-bold tracking-widest">
                        <MessageCircle className="w-3.5 h-3.5 text-emerald-400" />
                        <span>OFFICIAL COMMUNICATION CHANNEL</span>
                      </div>
                      <h4 className="text-lg sm:text-xl font-bold font-mono text-white uppercase tracking-tight">
                        JOIN THE OFFICIAL WHATSAPP GROUP
                      </h4>
                      <p className="text-xs font-sans text-gray-300 leading-relaxed">
                        Join the official Miraethon 2026 WhatsApp group for important announcements, updates, submission reminders and event information.
                      </p>
                    </div>

                    <a
                      href="https://chat.whatsapp.com/I0yvbjwTxEE8dfYViZ9Ev8?s=sh&p=a&ilr=4"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full sm:w-auto px-6 py-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-mono text-xs font-bold uppercase tracking-wider transition-all shadow-[0_0_20px_rgba(37,211,102,0.4)] hover:shadow-[0_0_30px_rgba(37,211,102,0.6)] flex items-center justify-center gap-2.5 shrink-0 group"
                    >
